...
/*
 * Layer Relay — central (dongle) side.
 *
 * When the active layer changes the dongle writes the highest active layer
 * index to ALL connected peripherals that expose the layer relay GATT
 * characteristic.  This lets the left half's nice!view display show the
...
static void relay_layer_to_all(uint8_t layer) {
    ...
    for (int i = 0; i < RELAY_MAX_CONNS; i++) {
        ...
        if (err) {
            LOG_DBG("layer relay write to slot %d err %d", i, err);
        }
    }
}
...
static uint8_t char_discover_cb(struct bt_conn *conn,
    ...
    for (int i = 0; i < RELAY_MAX_CONNS; i++) {
        ...
        if (attr == NULL) {
            LOG_DBG("layer relay char not found on slot %d", i);
        } else {
            struct bt_gatt_chrc *chrc = attr->user_data;
            relay_conns[i].handle = chrc->value_handle;
            LOG_INF("Layer relay char found on slot %d, handle 0x%04x",
                    i, relay_conns[i].handle);
            /* Immediately push the current layer to this peripheral */
...
static void start_discovery(struct relay_conn_info *info) {
    static const struct bt_uuid_128 relay_char_uuid =
        BT_UUID_INIT_128(LAYER_RELAY_CHAR_UUID);
...
BT_CONN_CB_DEFINE(layer_relay_conn_callbacks) = {
    .connected    = relay_connected,
    .disconnected = relay_disconnected,
};

/* ------------------------------------------------------------------ */
/* ZMK layer event listener                                             */
/* ------------------------------------------------------------------ */

static int layer_relay_event_handler(const zmk_event_t *eh) {
...
ZMK_LISTENER(layer_relay_central, layer_relay_event_handler);
ZMK_SUBSCRIPTION(layer_relay_central, zmk_layer_state_changed);

/* ------------------------------------------------------------------ */
/* Module init                                                          */
/* ------------------------------------------------------------------ */

static int layer_relay_central_init(void) {
    for (int i = 0; i < RELAY_MAX_CONNS; i++) {
        relay_conns[i].conn        = NULL;
        relay_conns[i].handle      = 0;
        relay_conns[i].discovering = false;
...
SYS_INIT(layer_relay_central_init, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);
